...
class Maker(object):
    ...
    def get_customer_effective_types(self):
        ...
        self.printer.debug((os.path.abspath(path)))
        for parent, dirnames, filenames in os.walk(path):
            self.printer.debug(parent)
            self.printer.debug(dirnames)
            self.printer.debug(filenames)
            for dirname in dirnames:
                self.printer.debug(dirname)
                self.printer.debug(config.device_types)
                if dirname not in config.device_types:
                    continue
                images.append(dirname)
                self.printer.debug(images)
    ...
    def pack(self):
        ...
        for i in range(0, packagecnt+1):
            ...
            subprocess.call(caller_zip, shell=True)
            if config.signature_flag:
                ...
                subprocess.call(caller_signature, shell=True)
    ...
    def cmdline_parse(self):
        ...
        parser.add_argument('-c', '--customerpath', help=help)
        ...
        if args.customerpath:
            config.Config.set_image_cfg_path(args.customerpath)
        ...
        self.printer.debug("image configuration file path: %s" %
                           (os.path.abspath(config.Config.get_image_cfg_path())))
        self.printer.debug("image file path: %s" %
                           (os.path.abspath(config.Config.get_image_path())))
        self.printer.debug("output directory path: %s" %
                           (os.path.abspath(config.Config.get_outputdir_path())))
        self.printer.debug("slice size: %d" % (config.Config.get_slicesize()))
        self.printer.debug("public key path: %s" %
                           (os.path.abspath(config.Config.get_public_key())))
        self.printer.debug("private key path: %s" %
                           (os.path.abspath(config.Config.get_private_key())))
        return self
